...
// Bluebonnet save current table 2
...
do {
	...
	if (array_key_exists($row_styles['id'], $styles_selected)) {
		...
		$style_value_edit = style_number_const($row_styles['brewStyleGroup'],$row_styles['brewStyleNum'],$_SESSION['style_set_system_separator'],1); // with leading zero
		...
		if ($action == "edit") {
			if ($row_styles['brewStyleGroup'].$row_styles['brewStyleNum'] == $row_log['brewCategorySort'].$row_log['brewSubCategory']) $selected_disabled = "SELECTED"; // BBO We are on the row being edited
			if (($row_styles['brewStyleGroup'].$row_styles['brewStyleNum'] != $row_log['brewCategorySort'].$row_log['brewSubCategory']) && ($subcat_limit)) $selected_disabled = "DISABLED"; // BBO Mark other rows as disabled if at the entrants subcat limit
...
// If doing an add, if the entrant has maxed out the entrant table limit they cannot select that style
	...
	// Bluebonnet end 4
		...
		// The current style is selected in edit, Geoff's code below BBO 5
...
// Bluebonnet 5
//print_r("BBOsubcat=$BBOsubcat, s-d=$selected_disabled, BBOtableLimitDisabled=$BBOtableLimitDisabled, action=$action 	// Bluebonnet
		...
		{
			if ($selected_disabled == "DISABLED") $selection .= " ".$brew_text_003; // geoff's code
		}
